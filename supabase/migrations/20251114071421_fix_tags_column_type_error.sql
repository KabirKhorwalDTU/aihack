/*
  # Fix Tags Column Type Error
  
  This migration fixes the type mismatch error where tags column is JSONB
  but we're trying to insert text array.
  
  1. Changes
    - Update batch_analyze_reviews function to convert text array to JSONB
...
BEGIN
  ...
  -- Step 8: Bulk update reviews with results (FIX: Convert array to JSONB)
